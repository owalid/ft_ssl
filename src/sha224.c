...
void    sha224_process_last_block(char *input, unsigned int *vars)
{
    ...
    sha224_process_firsts_blocks((unsigned int*)input, vars);
}
...
void    sha224_process(char *input)
{
    ...
    if (ft_strlen(input) >= 64) {
        while (current_len % 64 == 0) {
            ...
            sha224_process_firsts_blocks((unsigned int*)current_input, vars);
            input += current_len;
    ...
    // printf("%d\n", current_len);
    ft_strncpy(current_input, input, 64);
    sha224_process_last_block(current_input, vars);
